...
@section('main')
@php
    // Lấy danh sách màu đã chọn ở các biến thể trước
    $selectedColors = collect($product->productVariant)->pluck('color_id')->toArray();
@endphp
  <div class="container-fluid add-product">
    <form action = "{{route('UpdateProductById',$product->id)}}" method="POST" enctype="multipart/form-data">
      @csrf
      @method('PUT')
    <div class="row">
      <div class="col-xl-7">
        ...
        <div class="product-info">
          ...
          <div class="row">
            ...
            <div class="mb-3">
              ...
              <input class="form-control" name="name" value="{{old('name',$product->name)}}" placeholder="Enter Product Name" type="text">
              @error('name')
          ...
          <option value="{{$categories->id}}" {{ old('category_id', $product->category_id) == $categories->id ? 'selected' : '' }}>{{$categories->name}}</option>
            @endforeach
              </select>
               @error('category_id')
                 <span class="text-danger">{{$message}}</span>
            ...
            <div class="mb-3">
              ...
              <input class="form-control" value="{{$product->discount}}" name="discount" placeholder="10%" type="text"><span
          ...
          <div class="row">
            ...
            <div class="mb-3">
              ...
              <textarea id="title" name="title" value="{{old('title')}}"  class="form-control" placeholder="tiêu đề">
                    {{$product->title}}
              </textarea>
               @error('title')
                 <span class="text-danger">{{$message}}</span>
          ...
          <div class="row">
            ...
            <div class="mb-3">
              ...
              placeholder="miêu tả">{{$product->description}}</textarea>
               @error('description')
                 <span class="text-danger">{{$message}}</span>
              @enderror
            </div>
          ...
          <div class="dz-message needsclick">
            ...
            <div id="previewContainer2" class="flex flex-wrap mt-4 gap-2">
               @error('image_path')
                 <span class="text-danger">{{$message}}</span>
              @enderror
              @foreach ($product->AlbumImage as $album )
                    <img src="{{$album->image_path}}" width="100px" class="mx-1" height="100px" alt="">
              @endforeach
            </div>
          </div>
          </div>
      ...
      <div class="card">
        ...
        <div class="product-info">
          ...
          <div id="group-variant">
                @foreach ( $product->productVariant as $key=> $variant )
                    <div class="product-group my-1">
        <h4>Biến thể {{$key +1}}</h4>
            <div class="row">
            <div class="col-sm-6">
                <input type="text" name="variant[{{$key + 1}}][id]" value="{{$variant->id}}" hidden>
              <div class="mb-3">
              <label class="form-label">trọng lượng </label>
              <select name="variant[{{$key + 1}}][net_weight_id]" class="form-select">
                @foreach ( $net_weith as $net_weiths )
                      <option value="{{$net_weiths->id}}" {{$variant->net_weight_id === $net_weiths->id ? 'selected' : ''}}>{{$net_weiths->name}}</option>
                @endforeach
            ...
            <div class="col-sm-6">
              ...
              <select name="variant[{{$key + 1}}][color_id]" class="form-select">
                @foreach ( $color as $colors )
                     @php
                                // Chỉ hiển thị màu này nếu:
                                // - Là màu của biến thể hiện tại
                                // - Hoặc chưa được chọn ở các biến thể khác
                                $isSelected = $variant->color_id == $colors->id;
                                $isUsed = in_array($colors->id, $selectedColors) && !$isSelected;
                            @endphp
                            @if (!$isUsed)
                                <option value="{{$colors->id}}" {{ $isSelected ? 'selected' : '' }}>{{$colors->name}}</option>
                            @endif
                @endforeach
              </select>
            ...
            <div class="col-sm-6">
              ...
              <input class="form-control" value="{{$variant->stock}}"  name="variant[{{$key + 1}}][stock]" placeholder="Nhập số lượng ..." type="text"> 
            ...
            <div class="col-sm-6">
              ...
              <input class="form-control" value="{{$variant->price}}" name="variant[{{$key + 1}}][price]"  placeholder="Nhập giá ..." type="text"> 
              </div>
            </div>
            </div>
          </div>
...
@section('js')
  ...
  <script>
    ...
    const variant = @json($product->productVariant).length;

    // const net_weight_variant = @json($product->productVariant);
    let i = variant;
